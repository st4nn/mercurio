			...
			switch ($code) {
				case SPREADSHEET_EXCEL_READER_TYPE_SST:
					...
					for ($i = 0; $i < $uniqueStrings; $i++) {
						...
						$retstr = ($asciiEncoding) ? $retstr : utf8_encode($retstr);
				...
				case SPREADSHEET_EXCEL_READER_TYPE_BOUNDSHEET:
						...
						if ($version == SPREADSHEET_EXCEL_READER_BIFF8){
							...
							if ($chartype == 0){
								...
								$rec_name	= utf8_encode(substr($data, $pos+12, $rec_length*2));
							}
		...
		while($cont) {
			...
			switch ($code) {
				...
				case SPREADSHEET_EXCEL_READER_TYPE_HYPER:
					...
					$linkdata['link'] = utf8_encode($ulink);
					for ($r=$row; $r<=$row2; $r++) { 
